...
// Telemetry ingest enabled: authenticated ingest requests are parsed and written to Neon.
...
function intervalBucket(observedAt){const startMs=Math.floor(observedAt.getTime()/EXPERIMENT_STORE_INTERVAL_MS)*EXPERIMENT_STORE_INTERVAL_MS;return{start:new Date(startMs),end:new Date(startMs+EXPERIMENT_STORE_INTERVAL_MS)}}
...
export default async function handler(req,res){
  ...
  try{
    const sql=getSql();
    if(nodeNum===EXPERIMENT_NODE_NUM&&EXPERIMENT_TYPES.has(body.type)){
      const bucket=intervalBucket(observedAt);
      const existing=await sql`SELECT id FROM telemetry_readings WHERE node_num=${nodeNum} AND telemetry_type=${telemetryType} AND observed_at>=${bucket.start.toISOString()} AND observed_at<${bucket.end.toISOString()} LIMIT 1`;
      if(existing.length)return res.status(202).json({ok:true,stored:false,reason:`CCS3 ${telemetryType} already stored in this ${EXPERIMENT_STORE_INTERVAL_MINUTES}-minute interval`,bucket_start:bucket.start.toISOString()});
    }
    const rows=await sql`INSERT INTO telemetry_readings (observed_at,node_num,station_name,telemetry_type,temperature_c,metrics,radio,raw) VALUES (${observedAt.toISOString()},${nodeNum},${stationName},${telemetryType},${temperatureC},${JSON.stringify(metrics)}::jsonb,${JSON.stringify(radio)}::jsonb,${JSON.stringify(body)}::jsonb) RETURNING id,observed_at,station_name,telemetry_type,temperature_c,metrics,radio`;
    return res.status(201).json({ok:true,stored:true,reading:rows[0]});
  }catch(error){console.error('Telemetry ingest failed',error);return res.status(500).json({ok:false,error:'Database insert failed'})}
}
