...
export default async function handler(req, res) {
  ...
  if (!SUPPORTED.has(body.type)) {
    return res.status(202).json({ ok: true, stored: false, reason: 'Dashboard now accepts Hidden Valley telemetry only' });
  }

  const nodeNum = nodeNumber(body);
  if (nodeNum !== HIDDEN_VALLEY_NODE_NUM) {
    return res.status(202).json({ ok: true, stored: false, reason: 'Not Hidden Valley Repeater' });
  }
  ...
  if (body.type === 'telemetry') {
    ...
    if (temperatureC === null) {
      return res.status(202).json({ ok: true, stored: false, reason: 'Hidden Valley environmental packet has no temperature' });
    }
    ...
    if (batteryLevel === null && voltage === null) {
      return res.status(202).json({ ok: true, stored: false, reason: 'Hidden Valley device packet has no battery measurement' });
    }
