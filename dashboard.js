...
const reportedDryBaseline=r=>num(metric(r,'rock_dry_baseline_adc','dry_baseline_adc','baseline_adc'));
...
const motionMetric=r=>metric(r,'motion_detected','motion','pir','motion_active');
const motionHit=r=>bool(motionMetric(r));
...
const rawBatteryV=r=>num(metric(r,'battery_voltage','battery_voltage_v','voltage'));
const batteryV=r=>{const v=rawBatteryV(r);return v!==null&&v>=2.5&&v<=5.0?v:null};
const batteryPct=r=>{const p=num(metric(r,'battery_percent','battery_level','battery_pct')),raw=rawBatteryV(r);if(p===null||p<0||p>100)return null;if(raw!==null&&(raw<2.5||raw>5.0))return null;if(raw===0&&p===0)return null;return p};
...
function fmtMinute(iso){return new Date(iso).toLocaleString([], {month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}
function experimentRows(){return state.readings.filter(r=>num(r?.node_num)===EXPERIMENT_NODE_NUM)}
function rockRows(){return experimentRows().filter(r=>rockAdc(r)!==null)}
...
function renderSummary(){const rr=rockRows(),tr=temperatureRows(),rock=rr[0]||null,temp=tr[0]||null,latest=latestOverall();const adc=rockAdc(rock),volts=rockVoltage(rock),wet=calcWetness(rock),dry=reportedDryBaseline(rock)??TEMP_DRY_ADC,delta=adc===null?null:Math.max(0,dry-adc),trend=rockTrend(rr),band=deriveBand(rock),stateName=trend?.label||band;
  ...
  els.motionState.textContent=motionReading?(hit?'DETECTED':'CLEAR'):'—';els.motionState.className=hit?'warn':'good';els.motionDetail.textContent=motionReading?`${ageText(motionReading.observed_at)} since latest PIR telemetry · D6`:'PIR on D6';
  els.lastMotion.textContent=mt.detected?ageText(mt.detected.observed_at):'—';els.lastMotionDetail.textContent=mt.detected?`${fmtTime(mt.detected.observed_at)} · LOW→HIGH`:'no detected transition in selected window';
  els.lastClear.textContent=mt.cleared?ageText(mt.cleared.observed_at):'—';els.lastClearDetail.textContent=mt.cleared?`${fmtTime(mt.cleared.observed_at)} · HIGH→LOW`:'no clear transition in selected window';
  const b=rr.find(r=>batteryV(r)!==null||batteryPct(r)!==null)||null,bp=batteryPct(b),bv=batteryV(b);els.battery.textContent=bv!==null?`${bv.toFixed(2)} V`:bp!==null?`${Math.round(bp)}%`:'—';els.batteryDetail.textContent=bv!==null&&bp!==null?`${Math.round(bp)}% · field node`:bv!==null?'field node voltage':bp!==null?'mesh battery estimate':'waiting for corrected battery telemetry';
  if(latest){els.lastPacket.textContent=ageText(latest.observed_at);els.packetTime.textContent=fmtTime(latest.observed_at);const radio=latest.radio||{},rssi=num(radio.rssi),snr=num(radio.snr);els.lora.textContent=rssi===null?'—':`${Math.round(rssi)} dBm`;els.loraDetail.textContent=snr===null?'RSSI / SNR':`SNR ${snr.toFixed(1)} dB · ${radio.hops_away??'—'} hops`;const online=ageSeconds(latest.observed_at)<600;els.status.className=`live-pill ${online?'online':'offline'}`;els.statusText.textContent=online?'CCS3 experiment telemetry live':`Last CCS3 packet ${ageText(latest.observed_at)} ago`;}else{els.status.className='live-pill offline';els.statusText.textContent='Waiting for CCS3 data';els.lastPacket.textContent='—';els.packetTime.textContent='no telemetry yet';els.lora.textContent='—';els.loraDetail.textContent='RSSI / SNR';}
}
function renderExperimentStats(){const rows=rockRows(),values=rows.map(rockAdc).filter(Number.isFinite);if(!values.length){els.highestAdc.textContent=els.lowestAdc.textContent=els.adcSpan.textContent='—'}else{const hi=Math.max(...values),lo=Math.min(...values);els.highestAdc.textContent=Math.round(hi);els.lowestAdc.textContent=Math.round(lo);els.adcSpan.textContent=Math.round(hi-lo)}const totals=rows.map(motionTotal).filter(Number.isFinite);const hits=rows.filter(motionHit).length;els.motionCount.textContent=totals.length?Math.round(Math.max(...totals)):hits||'0'}
